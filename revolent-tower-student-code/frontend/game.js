...
class Game {
  constructor() {
    ...
    window.addEventListener("storage", () => {
      const action = JSON.parse(window.localStorage.getItem("action-end-game"));
      console.log(action);
      if (action == "1" && this.state === this.STATES.PLAYING) {
        this.placeBlock(true);
        this.endGame(false);
        window.localStorage.removeItem("action-end-game");
      }
    });
  ...
  startGame() {
    ...
    window.localStorage.removeItem("action-end-game");
    window.localStorage.removeItem("action-new-data");
  }
  restartGame() {
  ...
  placeBlock(endGame = false) {
    let currentBlock = this.blocks[this.blocks.length - 1];
    let newBlocks = currentBlock.place(endGame);
  ...
  addBlock() {
    ...
    if (lastBlock && lastBlock.state == lastBlock.STATES.MISSED) {
        console.log("5465465456445665465")
      return this.endGame();
    }
  ...
  endGame(update = true) {
    ...
    if (this.blocks.length > 2 && email && fullName && update) {
      window.parent.postMessage(
        JSON.stringify({
          type: "action-new-data",
          data: { email, fullName, score: this.blocks.length - 2 },
        })
      );
    }
